...
#ifndef BOING_KIT_QUATERNION
#define BOING_KIT_QUATERNION
...
{
  float h = 0.5 * a;
  return float4(sin(h) * normalize_safe(v), cos(h));
}

inline float4 quat_angular_vec(float3 v)
{
  return quat_axis_angle(v, length(v));
}
...
inline float4 quat_from_to(float3 from, float3 to)
{
  ...
  float3 axis = c / max(kEpsilon, sqrt(cc));
  float angle = acos(clamp(dot(normalize_safe(from), normalize_safe(to)), -1.0f, 1.0f));
...
inline float4 quat_basis(float3 xAxis, float3 yAxis, float3 zAxis)
{
  return quat_mat(mat_basis(xAxis, yAxis, zAxis));
}
