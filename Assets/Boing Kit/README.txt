...
Changes:
  ...
  Version 1.2.44:
   - Revert incorrect and unnecessary speculative "fix" for animation blend not being respected by rotational effects.

  Version 1.2.43:
   - Remove duplicate property sliders in shared params editor UI.
   - Replace usage of the array version of Mathf.Min/Max with VectorUtil.MinComponent/MaxComponent to remove garbage allocation.

  Version 1.2.42:
   - Add options to lock rotational effects along certain axes (in either global or local space).

  Version 1.2.41:
   - Add assembly definitions.

  Version 1.2.40:
   - Fix null reference exception in BoingBones.Reboot when instantiated at specfic timings.
   - Fix missing override keywords for BoingEffector.OnEnable/OnDisable.

  Version 1.2.39:
   - Fix invalid results pulled for objects not executed that frame (e.g. frome late enabling or instantiation).
   - Fix unnecessary execution of disabled objects.
   - Fix incorrect rotation computation introduced in previous update to respect animation blend.

  Version 1.2.38:
   - Fix animation blend not respected by rotational effects.
   - Remove garbage created by VectorUtil.MinComponent and VectorUtil.MaxComponent.
